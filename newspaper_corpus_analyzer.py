...
# Newspaper codes and full names from the PDF
NEWSPAPER_NAMES = {
...
class NewspaperCorpusAnalyzer:
    ...
    def parse_filename(self, filename):
        ...
        # Pattern: 3-letter code, date (YYYY_MM_DD), page (3 digits), library (2 letters), optional shelfmark
        pattern = r'^([A-Z]{3})_(\d{4})_(\d{2})_(\d{2})_(\d{3})_([A-Z]{2})(?:_(.+))?$'
    ...
    def scan_corpus(self):
        ...
        for img_file in self.corpus_root.rglob('*'):
            ...
            if not img_file.is_file():
                continue
            if img_file.suffix.lower() not in ['.jpg', '.jpeg', '.png', '.tif', '.tiff', '.pdf']:
                continue
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python newspaper_corpus_analyzer.py <corpus_root_directory>")
        print("\nExample: python newspaper_corpus_analyzer.py /path/to/newspaper/corpus")
        sys.exit(1)
